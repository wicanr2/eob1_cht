...
bool Screen_EoB::init() {
	if (Screen::init()) {
		...
		if ((_vm->game() == GI_EOB2 || _vm->game() == GI_EOB1) && _vm->gameFlags().lang == Common::Language::ZH_TWN) {
			Common::File f;
			if (!f.open("ceob.pat"))
				return false;
...
bool Screen_EoB::loadFont(FontId fontId, const char *filename) {
	...
	} else if (fontId == FID_CHINESE_FNT && (_vm->game() == GI_EOB2 || _vm->game() == GI_EOB1) && _vm->gameFlags().lang == Common::ZH_TWN) {
