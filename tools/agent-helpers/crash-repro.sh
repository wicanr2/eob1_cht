#!/bin/bash
# Isolated crash repro driver — uses DISPLAY :88, log /tmp/eob1-crash/
# Avoids stomping on iter5 tester at :99.
set -u
DISPLAY_NUM=88
LOG=/tmp/eob1-crash/scummvm.log
SCUMMVM=/root/scummvm_work/scummvm/scummvm
GAMEDIR=/root/eob1cht
SHOT_DIR=/tmp/eob1-crash/shots
WID_FILE=/tmp/eob1-crash/wid
PID_FILE=/tmp/eob1-crash/pid
mkdir -p /tmp/eob1-crash $SHOT_DIR
export DISPLAY=:$DISPLAY_NUM

start() {
    pkill -9 -f "scummvm.*--path=$GAMEDIR" 2>/dev/null
    pkill -9 -f "Xvfb :$DISPLAY_NUM" 2>/dev/null
    sleep 1
    rm -f $WID_FILE $PID_FILE
    Xvfb :$DISPLAY_NUM -nolisten unix -screen 0 800x600x24 +extension GLX >/tmp/eob1-crash/xvfb.log 2>&1 &
    sleep 1
    > $LOG
    ...
    disown
    sleep 6
    SC_PID=$(pgrep -f "scummvm.*--extrapath=$GAMEDIR" | head -1)
...
shot() {
    ...
    if [ -f "$OUT" ]; then echo "snap $OUT ($(stat -c %s "$OUT"))"; else echo "snap FAILED $OUT"; fi
}

key() {
    if ! alive; then echo "DEAD-pre-key $*"; return 1; fi
    local WID=$(cat $WID_FILE)
    for k in "$@"; do timeout 0.5 xdotool key --window $WID --delay 10 "$k" 2>/dev/null; sleep 0.05; done
}
...
logtail() { tail -30 $LOG; }
status() { alive && echo "alive pid=$(cat $PID_FILE)" || echo "DEAD"; }
